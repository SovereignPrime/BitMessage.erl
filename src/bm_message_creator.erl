...
-spec create_message(Command, Payload) -> message_bin() when
      Command :: binary(),
      Payload :: binary().
...
-spec create_obj([Hash]) -> message_bin() when 
      Hash ::binary().
create_obj(Hash) ->  % {{{1
...
-spec create_inv(Hashes) -> message_bin() when
      Hashes :: [Hash],
      Hash :: binary().
create_inv(Hashes) ->  % {{{1
...
-spec create_big_inv(Stream, Exclude) -> message_bin() when
      Stream :: integer(),
      Exclude :: list().
...
-spec create_addrs_for_stream(Stream) -> message_bin() when
      Stream :: integer().
