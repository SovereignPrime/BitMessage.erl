...
integer_to_bytes(_Config) ->  % {{{2
        ?_assert(integer_to_bytes(crypto:bytes_to_integer(<<"TEST">>)) == <<"TEST">>).

decode_encode_list() ->  % {{{2
    [].

decode_encode_list(_Config) ->  % {{{2
    ok=eunit:test({generator, fun decode_encode_list_test_/0}).

decode_encode_network() ->  % {{{2
    [].
decode_encode_network(_Config) ->  % {{{2
    ok=eunit:test({generator, fun decode_encode_network_test_/0}).

decode_encode_varint() ->  % {{{2
    [].
decode_encode_varint(_Config) ->  % {{{2
    ok=eunit:test({generator, fun decode_encode_varint_test_/0}).

decode_encode_varstr() ->  % {{{2
    [].
decode_encode_varstr(_Config) ->  % {{{2
    ok=eunit:test({generator, fun decode_encode_varstr_test_/0}).

encode_list() ->  % {{{2
    [].
encode_list(_Config) ->  % {{{2
    ok=eunit:test({generator, fun encode_list_test_/0}).

encode_network() ->  % {{{2
    [].
encode_network(_Config) ->  % {{{2
    ok=eunit:test({generator, fun encode_network_test_/0}).

encode_varint() ->  % {{{2
    [].

encode_varint(_Config) ->  % {{{2
    ok=eunit:test({generator, fun encode_varint_test_/0}).

encode_varstr() ->  % {{{2
    [].

encode_varstr(_Config) ->  % {{{2
    ok=eunit:test({generator, fun encode_varstr_test_/0}).

pow() ->  % {{{2
    [].

pow(_Config) ->  % {{{2
    ok=eunit:test({generator, fun pow_test_/0}).

sqrt() ->  % {{{2
    [].

sqrt(_Config) ->  % {{{2
    ok=eunit:test({generator, fun sqrt_test_/0}).
...
pow_test_() ->  % {{{2
    [
        ?_assertEqual(bm_types:pow(2, 2), 4),
        ?_assertEqual(bm_types:pow(3, 2), 9),
        ?_assertEqual(bm_types:pow(4, 2), 16),
        ?_assertEqual(bm_types:pow(5, 2), 25),
        ?_assertEqual(bm_types:pow(2, 3), 8),
        ?_assertEqual(bm_types:pow(2, 4), 16),
        ?_assertEqual(bm_types:pow(2, 5), 32)
    ].

sqrt_test_() ->  % {{{2
    [
        ?_assertEqual(bm_types:sqrt(1), 1),
        ?_assertEqual(bm_types:sqrt(4), 2),
        ?_assertEqual(bm_types:sqrt(9), 3),
        ?_assertEqual(bm_types:sqrt(16), 4),
        ?_assertEqual(bm_types:sqrt(25), 5)
    ].
